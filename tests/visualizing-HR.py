...
def save_gradient_plot(gradient, mesh, output_dir: str):
    ...
    ax.set(xlabel='Offset [km]', ylabel='Depth [km]',
           title='Gradient - iter#1')
    ax.set_aspect('auto')
    # Make a colorbar for the ContourSet returned by the contourf call.
    cbar = fig.colorbar(aa)
    cbar.ax.set_ylabel('Velocity [km/s]')
...
def save_adjoint_source_plot(adj, ext: list, clim: list, output_dir: str):
    ...
    aa = plt.imshow(adj, interpolation='nearest', aspect='auto', cmap='gray', clim=clim,
                    extent=ext)
    ax.set(xlabel='Offset [km]', ylabel='Time [s]',
           title='Adjoint sources of the first iteration -- L2')
    ax.set_aspect('auto')
    # Make a colorbar for the ContourSet returned by the contourf call.
    cbar = fig.colorbar(aa)
    cbar.ax.set_ylabel('Amplitude')
...
def get_velocity_profiles(base_dir: str, pattern: str, mesh):
    ...
    res_file_list = [os.path.join(base_dir, x) for x in os.listdir(
        base_dir) if compiled_pattern.match(x)]

    # Iteration number list
    it_nbrs = ['iter_'+x.split('.')[0][2:] for x in res_file_list]

    X = [loadmat(x) for x in res_file_list]
    V = [np.reshape(x['data'], mesh.shape(as_grid=True)).transpose()
         for x in X]
...
if __name__ == "__main__":
    ...
    for vm in velocity_models:
        ...
        save_velocity_profile_plot(
            velocity_models[vm], vm, m, clim, abs_path_name)

    # Velocity traces
    n_tr = 5

    # Velocity traces from normal models
    save_velocity_traces(velocity_models, X_coord, Z_coord,
                         n_tr, 'velocity_trace_', p_dir)

    # Velocity traces at different iterations
    res_name = '^x_[0-9]+.mat'
    V = get_velocity_profiles(res_dir, res_name, m)
    save_velocity_traces(V, X_coord, Z_coord, n_tr,
                         'iter_velocity_trace_', p_dir)
    ...
    save_wavefield_plot(
        wavefields['True']-wavefields['Inverted'], 'diff', extent, clim, p_dir)
