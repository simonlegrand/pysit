...
if __name__ == "__main__":
    ...
    clim=[-0.2*wavefields['True'].max(),0.2*wavefields['True'].max()]
    clim_diff=[-0.05*wavefields['True'].max(), 0.05*wavefields['True'].max()]
    for wf in wavefields:
        save_wavefield_plot(wavefields[wf], wf, extent, clim, p_dir)

    save_wavefield_plot(wavefields['True']-wavefields['Inverted'], 'diff', extent, clim_diff, p_dir)
