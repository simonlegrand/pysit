...
if __name__ == '__main__':
    ...
    if rank == 0:
        ttt = time.time()
        sys.stdout.write('Linear increased velocity model \n')
    ...
    C, C0, m, d = linear_increasing_velocity(model_param_set=m_param)

    if rank == 0:
        sys.stdout.write('vp_true = %s \n' %m_param['vp_true'])
        sys.stdout.write('alpha_true = %s \n' %m_param['alpha_true'])
        sys.stdout.write('vp_initial = %s \n' %m_param['vp_initial'])
        sys.stdout.write('alpha_initial = %s \n' %m_param['alpha_initial'])
